...
class SDFNetwork(nn.Module):
    ...
    def __init__(self, d_in, d_out, d_hidden, n_layers, skip_in=(4,), multires=0, bias=0.5, scale=1, geometric_init=True, weight_norm=True, inside_outside=False):
        ...
        for l in range(0, self.num_layers - 1):
            if l + 1 in self.skip_in: out_dim = dims[l + 1] - dims[0]
            else: out_dim = dims[l + 1]
    ...
    def forward(self, inputs):
        ...
        for l in range(0, self.num_layers - 1):
            ...
            if l in self.skip_in: x = torch.cat([x, inputs], 1) / np.sqrt(2)
            x = lin(x)
            if l < self.num_layers - 2: x = self.activation(x)
        return torch.cat([x[:, :1] / self.scale, x[:, 1:]], dim=-1)

    def sdf(self, x): return self.forward(x)[:, :1]
    def sdf_hidden_appearance(self, x): return self.forward(x)
    def gradient(self, x):
...
class SDFNetworkHigh(nn.Module):
    ...
    def forward(self, inputs, sdf_network, gradient, s):
        ...
        sdf_output = sdf_network(pts.reshape(-1, 3) - weaken * x[:, :1] * (gradient / (torch.norm(gradient,dim=1,keepdim=True) + 1e-6)))
...
class RenderingNetwork(nn.Module):
    ...
    def forward(self, points, normals, view_dirs, feature_vectors):
        ...
        for l in range(0, self.num_layers - 1):
            lin = getattr(self, "lin" + str(l))
            x = lin(x)
            if l < self.num_layers - 2: x = self.relu(x)
        if self.squeeze_out: x = torch.sigmoid(x)
        return x
