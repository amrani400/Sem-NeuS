...
from semantic.preprocess import extract_and_save_features
...
class Dataset:
    def __init__(self, conf):
        ...
        # DINO EXTRACTION 
        feature_dir = os.path.join(self.data_dir, 'dinov3_features')
        existing_feats = glob(os.path.join(feature_dir, '*.pt'))
        
        
        if len(existing_feats) != self.n_images:
            print(f"Dataset Info: Found {len(existing_feats)} features for {self.n_images} images.")
            print("Triggering automatic DINOv3 feature extraction...")
            ckpt_path = './dinov3_vitl16.pth' 
            
            extract_and_save_features(self.data_dir, ckpt_path)
            
            existing_feats = glob(os.path.join(feature_dir, '*.pt'))

        # LOAD FEATURES I
        if len(existing_feats) == self.n_images:
            print(f"Loading {len(existing_feats)} DINOv3 feature maps into GPU memory...")
            sorted_feats = sorted(existing_feats)
            self.dino_features = [torch.load(f).to(self.device).float() for f in sorted_feats]
            print("DINO Features loaded successfully.")
        else:
            print("WARNING: Feature extraction failed or model missing. Using Dummy Zero features.")
            self.dino_features = [torch.zeros(1024, 64, 64).float().to(self.device) for _ in range(self.n_images)]
